...
def main():
    ...
    # 解析命令行参数
    parser = argparse.ArgumentParser(description="Taxonomy扩展任务评估系统")
    ...
    # 加载扩展任务数据集
    print("\n加载扩展任务数据集...")
    expansion_data = data_loader.load_expand_data("data/data3/taxonomy_expansion.json")

    if not expansion_data:
        print("错误：无法加载数据集")
        exit(1)
        
    print(f"成功加载 {len(expansion_data)} 个样本")

    # 选择样本
    if args.samples is not None:
        test_samples = expansion_data[:args.samples]
    else:
        test_samples = expansion_data

    print(f"将处理 {len(test_samples)} 个样本")

    # 所有策略
    # strategies = ["zero_shot", "few_shot", "cot"]
    strategies = ["few_shot", "cot"]
    all_results = {}
    
    # 创建输出目录
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    output_dir = f"new_results3/expansion_run_{timestamp}_{api_config['model']}"
    ...
    for strategy in strategies:
        ...
        for i, sample in enumerate(test_samples):
            try:
                ...
                # 处理样本
                result = processor.process_taxonomy_expansion(
        ...
        # 保存最终结果
        try:
            output_path = os.path.join(output_dir, f"expansion_{strategy}_results.json")
            processor.save_results(results, output_path)
            log_message(f"\n{strategy} 策略结果已保存至: {output_path}")
            
            # 保存详细信息
            details_path = os.path.join(output_dir, f"expansion_{strategy}_details.json")
